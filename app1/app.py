...
def increment_time():
    global current_time, drift, master_clock, clock1_time
    while True:
        with lock:
            current_time += timedelta(seconds=1 + drift)
            clock1_time = current_time.strftime("%H:%M:%S")
        time.sleep(1)
...
def send_time():
    ...
    while True:
        ...
        try:
            response2 = requests.post('http://127.0.0.1:8001/update_time', json={'time': current_time_str, 'id': clock_id})
            response3 = requests.post('http://127.0.0.1:8002/update_time', json={'time': current_time_str, 'id': clock_id})
            print(f"Sent time to clock2: {response2.status_code}, clock3: {response3.status_code}")
...
def get_time():
    ...
    return jsonify(clock1=time_str, clock2=clock2_time, clock3=clock3_time)
