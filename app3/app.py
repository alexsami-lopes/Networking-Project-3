...
def increment_time():
    global current_time, drift, master_clock, clock3_time
    while True:
        with lock:
            current_time += timedelta(seconds=1 + drift)
            clock3_time = current_time.strftime("%H:%M:%S")
        time.sleep(1)
...
def send_time():
    ...
    while True:
        ...
        try:
            ...
            print(f"Sent time to clock1: {response1.status_code}, clock2: {response2.status_code}")
...
def get_time():
    ...
    return jsonify(clock1=clock1_time, clock2=clock2_time, clock3=time_str)
